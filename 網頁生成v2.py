...
def normalize_data(data):
    ...
    # 1. 處理草莓品種 (來自 strawberry_varieties.json)
    ...
    # 2. 處理病蟲害 (來自 strawberry_disease.json)
    ...
    # 3. 處理缺素 (來自 strawberry_disease.json)
    ...
    <script>
        function appData() {{
            return {{
                ...
                get minPercent() {{
                    return (Math.min(this.minSugar, this.maxSugar) / this.rangeMax) * 100 + '%';
                }},
                get maxPercent() {{
                    return (Math.max(this.minSugar, this.maxSugar) / this.rangeMax) * 100 + '%';
                }},
                get fillWidth() {{
                ...
                get filteredItems() {{
                    ...
                    return this.rawData.filter(item => {{
                        ...
                        if (this.activeCategory === '品種' || this.activeCategory === 'all') {{
                            ...
                            if (s === 0) {{
                                matchSugar = (currentMin === 0);
                            }} else {{
                                matchSugar = s >= currentMin && s <= currentMax;
                            }}
                        }}
            ...
            <div x-show="activeCategory === 'all' || activeCategory === '品種'" class="mb-6 pb-6 border-b border-gray-100 dark:border-gray-800 px-2 md:px-8">
                ...
                <div class="range-slider-container mb-2">
                    ...
                    <div class="range-track-fill" 
                         :style="'left: ' + minPercent + '; width: ' + fillWidth"></div>
                    
                    <input type="range" min="0" max="20" step="0.5" 
                           x-model.number="minSugar" 
                           @input="validateRange()"
                           class="range-slider-input">
                    
                    <input type="range" min="0" max="20" step="0.5" 
                           x-model.number="maxSugar"
                           @input="validateRange()" 
                           class="range-slider-input">
                </div>
                            ...
                            <div class="w-full h-full relative">
                                <template x-for="(img, idx) in item.images" :key="idx">
                                    <img :src="img" 
                                         ...
                                         @click="window.open(img, '_blank')" alt="Image">
                                </template>
                                <div x-show="item.images.length > 1" class="absolute inset-0 flex justify-between items-center px-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button @click.stop="currentImgIdx = (currentImgIdx - 1 + item.images.length) % item.images.length" class="bg-black/50 text-white p-2 rounded-full hover:bg-black/70 w-8 h-8 flex items-center justify-center"><i class="fa-solid fa-chevron-left text-xs"></i></button>
                                    <button @click.stop="currentImgIdx = (currentImgIdx + 1) % item.images.length" class="bg-black/50 text-white p-2 rounded-full hover:bg-black/70 w-8 h-8 flex items-center justify-center"><i class="fa-solid fa-chevron-right text-xs"></i></button>
                                </div>
                            </div>
                        </template>
                        <template x-if="!item.images || item.images.length === 0">
